...
(function(Opal) {
  ...
  Opal.add_stubs(['$require', '$attr_accessor', '$==', '$new', '$puts', '$name', '$admin?']);
  self.$require("opal");
  (function($base, $super) {
    function $User(){};
  ...
  self.$puts(user.$name());
  self.$puts("Is Bob an admin? " + (user['$admin?']()));
  user = $scope.get('User').$new("Admin");
  self.$puts(user);
  return self.$puts("Is Admin an admin? " + (user['$admin?']()));
})(Opal);
